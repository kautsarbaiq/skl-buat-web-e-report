...
<body>
  ...
  <!-- Content -->
  <div class="w-full pt-10 px-4 sm:px-6 md:px-8 lg:ps-72 mx-auto">
    <!-- ======= button atas ===== -->
    <button data-modal-target="popup-modal-3" data-modal-toggle="popup-modal-3" class="btn btn-accent mt-12 bg-white hover:bg-white border-indigo-50 hover:border-white">Buat postingan</button>
    <div id="popup-modal-3" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
      <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
          <button data-modal-hide="popup-modal-3" type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="popup-modal">
            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
              <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
            </svg>
            <span class="sr-only" data-modal-hide="popup-modal-3" type="button">Close modal</span>
          ...
          <div class="p-4 md:p-5 text-center">
            ...
            <form class="max-w-sm mx-auto" method="post" action="/service/function.php" enctype="multipart/form-data">
              ...
              <button name="submit-report" data-modal-hide="popup-modal-3" type="submit" class="text-white bg-green-600 hover:bg-green-300 focus:ring-4 focus:outline-none focus:ring-white-300 dark:focus:ring-white-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                Upload file
              </button>
              <button data-modal-hide="popup-modal-3" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">No, cancel</button>
    ...
    <!-- ====== card ======== -->
    <div class="grid grid-cols-3 justify-around mt-16 gap-y-6">
      <?php
      foreach ($reports as $laporan):
      ?>

        <div class="card glass w-3/4 h-1/2">
          <figure>
            <img class="w-fit h"
              src="/img/<?= $laporan['img'] ?>"
              alt="car!" />
          ...
          <div class="card-body">
            <h2 class="card-title"><?= $laporan['judul'] ?></h2>
            <p><?= $laporan['descripsi'] ?></p>
            <div class="card-actions justify-end">
              ...
              <!-- lihat -->
              <button data-modal-target="popup-modal-<?= $laporan['id'] ?>" data-modal-toggle="popup-modal-<?= $laporan['id'] ?>" class="btn btn-accent mt-12 bg-white hover:bg-white border-indigo-50 hover:border-white">Edit</button>
              <div id="popup-modal-<?= $laporan['id'] ?>" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative p-4 w-full max-w-md max-h-full">
                  <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
